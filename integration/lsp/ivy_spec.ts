...
use(chaiLike.default);
use(chaiThings.default);

describe('Angular Ivy language server', function() {
  ...
  it('should send ngcc progress after a project has finished loading', async () => {
    ...
    expect(configFilePath.endsWith('integration/project/tsconfig.json')).to.equal(true);
  });
  ...
  it('should disable language service until ngcc has completed', async () => {
    ...
    expect(languageServiceEnabled).to.equal(false);
  });

  it('should re-enable language service once ngcc has completed', async () => {
    openTextDocument(client, APP_COMPONENT);
    const languageServiceEnabled = await waitForNgcc(client);
    expect(languageServiceEnabled).to.equal(true);
  });
  ...
  it('should handle hover on inline template', async () => {
    ...
    expect(response?.contents).be.an('array').that.contains.something.like({
      language: 'typescript',
  ...
  it('should show diagnostics for external template on open', async () => {
    ...
    expect(diagnostics[0].message)
        .to.equal(`Property 'doesnotexist' does not exist on type 'FooComponent'.`);
    expect(diagnostics[0].relatedInformation).not.to.be.undefined;
    expect(diagnostics[0].relatedInformation!.length).to.equal(1);
    expect(diagnostics[0].relatedInformation![0].message)
        .to.equal(`Error occurs in the template of component FooComponent.`);
    expect(diagnostics[0].relatedInformation![0].location.uri).to.equal(FOO_COMPONENT_URI);
  });
  ...
  it('should support request cancellation', async () => {
    ...
    await expect(promise).to.eventually.rejected;
  });
  ...
  it('does not break after opening `.d.ts` file from external template', async () => {
    ...
    expect(targetUri.endsWith('angular/common/index.d.ts')).to.equal(true);
    // Open the `.d.ts` file
    ...
    expect(hoverResponse?.contents).to.be.an('array').that.contains.something.like({
      language: 'typescript',
  ...
  it('goes to definition of original source when compiled with source maps', async () => {
    ...
    expect(Array.isArray(response)).to.equal(true);
    const {targetUri} = response[0];
    expect(targetUri).to.contain('libs/post/src/lib/post.component.ts');
  });
  ...
  describe('signature help', () => {
    it('should show signature help for an empty call', async () => {
      ...
      expect(response.signatures.length).to.equal(1);
      expect(response.signatures[0].label).to.contain('(): string');
    });
    ...
    it('should show signature help with multiple arguments', async () => {
      ...
      expect(response.signatures[0].label)
          .to.contain('(from: number, length?: number | undefined): string');
      expect(response.signatures[0].parameters).not.to.be.undefined;
      expect(response.activeParameter).to.equal(1);
      ...
      expect(paramLabels).contains('from: number', 'length?: number | undefined');
    });
  });
  ...
  describe('completions', () => {
    it('for events', async () => {
      ...
      expect(outputCompletion.kind).to.equal(lsp.CompletionItemKind.Property);
      // // replace range includes the closing )
      expect((outputCompletion.textEdit as lsp.TextEdit).range)
          .to.be.like({start: {line: 0, character: 8}, end: {line: 0, character: 10}});
    });
  });
  ...
  describe('renaming', () => {
    describe('from template files', () => {
      ...
      it('should handle prepare rename request for property read', async () => {
        ...
        expect(response.range).to.be.like({
          start: {line: 0, character: 2},
          end: {line: 0, character: 7},
        });
        expect(response.placeholder).to.equal('title');
      });
      ...
      it('should handle rename request for property read', async () => {
        ...
        expect(response).not.to.equal(null);
        expect(response?.changes?.[FOO_TEMPLATE_URI].length).to.equal(1);
        expect(response?.changes?.[FOO_TEMPLATE_URI]).to.be.an('array').that.contains.something.like(expectedRenameInTemplate);
        expect(response?.changes?.[FOO_COMPONENT_URI].length).to.equal(1);
        expect(response?.changes?.[FOO_COMPONENT_URI]).to.be.an('array').that.contains.something.like(expectedRenameInComponent);
      });
    });
    ...
    describe('from typescript files', () => {
      ...
      it('should handle prepare rename request for inline template property read', async () => {
        ...
        expect(response.placeholder).to.equal('name');
      });
      ...
      describe('property rename', () => {
        ...
        it('should handle rename request for property read in a template', async () => {
          ...
          expect(response).not.to.equal(null);
          expect(response?.changes?.[APP_COMPONENT_URI].length).to.equal(2);
          expect(response?.changes?.[APP_COMPONENT_URI]).to.be.an('array').that.contains.something.like(expectedRenameInComponent);
          expect(response?.changes?.[APP_COMPONENT_URI]).to.be.an('array').that.contains.something.like(expectedRenameInTemplate);
        });
  ...
  describe('compiler options', () => {
    ...
    describe('strictTemplates: false', () => {
      ...
      it('should disable renaming when strict mode is disabled', async () => {
        ...
        expect(prepareRenameResponse).to.equal(null);
        ...
        expect(renameResponse).to.equal(null);
      });
    });
  });
  ...
  it('should handle GetTemplateLocationForComponent request', async () => {
    ...
    expect(response).not.to.be.undefined;
    expect(response.uri).to.contain('foo.component.html');
  });
  ...
  it('should provide a "go to component" codelens', async () => {
    ...
    expect(codeLensResponse).not.to.be.undefined;
    const [codeLens] = codeLensResponse!;
    expect(codeLens.data.uri).to.equal(FOO_TEMPLATE_URI);

    const codeLensResolveResponse =
        await client.sendRequest(lsp.CodeLensResolveRequest.type, codeLensResponse![0]);
    expect(codeLensResolveResponse).not.to.be.undefined;
    expect(codeLensResolveResponse?.command?.title).to.equal('Go to component');
  });
  ...
  it('detects an Angular project', async () => {
    ...
    expect(templateResponse).to.equal(true);
    const componentResponse = await client.sendRequest(IsInAngularProject, {
      textDocument: {
        uri: FOO_COMPONENT_URI,
      }
    });
    expect(componentResponse).to.equal(true);
  })
});

describe('auto-apply optional chaining', function() {
  this.timeout(10_000);
  ...
  it('should work on nullable symbol', async () => {
    ...
    expect(completion.kind).to.equal(lsp.CompletionItemKind.Property);
    expect((completion.textEdit as lsp.TextEdit).newText).to.equal('?.name');
  });
  ...
  it('should work on NonNullable symbol', async () => {
    ...
    expect(completion.kind).to.equal(lsp.CompletionItemKind.Method);
    expect((completion.textEdit as lsp.TextEdit).newText).to.equal('substr');
  });
});

describe('insert snippet text', function() {
  this.timeout(10_000);
  ...
  it('should be able to complete for an attribute with the value is empty', async () => {
    ...
    expect(completion.kind).to.equal(lsp.CompletionItemKind.Property);
    expect(completion.insertTextFormat).to.equal(lsp.InsertTextFormat.Snippet);
    expect((completion.textEdit as lsp.TextEdit).newText).to.equal('(appOutput)="$1"');
  });
  ...
  it('should not be included in the completion for an attribute with a value', async () => {
    ...
    expect(completion.kind).to.equal(lsp.CompletionItemKind.Property);
    expect(completion.insertTextFormat).to.be.undefined;
    expect((completion.textEdit as lsp.TextEdit).newText).to.equal('appInput');
  });
});
